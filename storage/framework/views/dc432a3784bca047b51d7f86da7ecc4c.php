...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div id="mobile-money-fields" class="row g-3" style="display: none;">
                            <div class="col-12">
                                <div class="alert alert-secondary border-0 bg-light p-3">
                                    <div id="orange-instructions" style="display: none;">
                                        <h6 class="fw-bold text-orange"><i class="fas fa-mobile-alt me-2"></i>Orange Money</h6>
                                        <p class="mb-2">Composez : <code class="bg-white p-1 rounded fw-bold" style="font-size: 1.1rem;">*144*10*04370203*Mt#</code></p>
                                    </div>
                                    <div id="moov-instructions" style="display: none;">
                                        <h6 class="fw-bold text-primary"><i class="fas fa-mobile-alt me-2"></i>Moov Money</h6>
                                        <p class="mb-2">Composez : <code class="bg-white p-1 rounded fw-bold" style="font-size: 1.1rem;">*555*4*1*O3239643*Mt#</code></p>
                                    </div>
                                    <small class="text-muted"><i class="fas fa-info-circle me-1"></i>Remplacez <span class="badge bg-white text-dark fw-bold border">Mt</span> par votre montant en chiffres.</small>
                                    <div class="mt-2 text-warning fw-bold small">
                                        <i class="fas fa-exclamation-triangle"></i> Important : Utilisez obligatoirement votre numéro de téléphone enregistré sur SIFCash pour effectuer le transfert.
                                    </div>
                                </div>
                            </div>
                            <div class="col-12 col-md-6">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    modePaiementSelect.addEventListener('change', function() {
        ...
        document.getElementById('orange-instructions').style.display = 'none';
        document.getElementById('moov-instructions').style.display = 'none';

        const preuveField = document.getElementById('preuve').closest('.mb-3');
        if(selectedMode === 'espece') {
            preuveField.style.display = 'none';
            document.getElementById('mobile-money-fields').style.display = 'none';
        } else {
            preuveField.style.display = 'block';
        }

        if(selectedMode === 'orange_money' || selectedMode === 'moov_money') {
            document.getElementById('mobile-money-fields').style.display = 'block';
            const refInput = document.getElementById('reference_paiement');
            
            if(selectedMode === 'orange_money') {
                document.getElementById('orange-instructions').style.display = 'block';
                refInput.placeholder = "Ex: MP260323.1558.68192189";
            } else {
                document.getElementById('moov-instructions').style.display = 'block';
                refInput.placeholder = "Ex: 260323155868";
            }
        ...
        adhesionSelect.addEventListener('change', function() {
            ...
            if (selectedOption && selectedOption.value) {
                ...
                if (plan && montant) {
                    ...
                    // Pré-remplissage automatique des champs si l'utilisateur change l'adhésion
                    // 1. Montant
                    document.getElementById('montant').value = montant;
                    
                    // 2. Catégorie (passer à 'cotisation' par défaut car c'est une adhésion existante)
                    const categorySelect = document.getElementById('categorie');
                    if (!categorySelect.value || categorySelect.value === 'ouverture') {
                        categorySelect.value = 'cotisation';
                    }
                    
                    // 3. Mode de paiement (utiliser le dernier utilisé)
                    const lastMode = selectedOption.dataset.lastMode;
                    if (lastMode) {
                        modePaiementSelect.value = lastMode;
                        // Déclencher l'événement change pour afficher les champs correspondants (mobile money, etc.)
                    ...
                    const labelPeriodicite = periodiciteLabels[periodicite] || periodicite;
                        ...
                        <div class="border rounded p-3 bg-light text-start">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-12 mt-2">
                                    <span class="badge bg-soft-info text-info border border-info-subtle" style="background-color: #e0f2fe;">
                                        <i class="fas fa-history me-1"></i>Renouvellement : ${labelPeriodicite}
                                    </span>
                                </div>
                            </div>
                        </div>
