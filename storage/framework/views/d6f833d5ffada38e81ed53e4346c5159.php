<div class="position-sticky pt-3">
    ...
    <ul class="nav flex-column">
        ...
        <!-- Section Versements (Paiements) -->
        <li class="nav-item">
            <a class="nav-link d-flex align-items-center justify-content-between <?php echo e(request()->routeIs('admin.paiements.*') ? 'active' : ''); ?>" 
               data-bs-toggle="collapse" href="#" data-bs-target="#paiementsMenu" role="button" aria-controls="paiementsMenu" aria-expanded="<?php echo e(request()->routeIs('admin.paiements.*') ? 'true' : 'false'); ?>">
                <div class="d-flex align-items-center">
                    <i class="fas fa-file-invoice-dollar me-3"></i>
                ...
                <?php
                    $u = Auth::user();
                    $nbPaiementsPending = 0;
                    if ($u) {
                        if ($u->isAdmin()) {
                            $nbPaiementsPending = \App\Models\Paiement::where('statut','soumis')->count();
                        } elseif ($u->isChefService()) {
                            $nbPaiementsPending = \App\Models\Paiement::where('statut','soumis')->whereHas('adherent', fn($q)=>$q->where('agence_id', $u->agence_id))->count();
                        } elseif ($u->isAgent()) {
                            $nbPaiementsPending = \App\Models\Paiement::where('statut','soumis')->whereHas('adherent.agents', fn($q)=>$q->where('agent_id', $u->id))->count();
                        }
                    }
                ?>
                <?php if(($nbPaiementsPending ?? 0) > 0): ?>
                    <span class="badge rounded-pill px-2 py-1 shadow-sm" 
                          style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%) !important; animation: pulse 2s infinite; color: white;">
                        <?php echo e($nbPaiementsPending); ?>

                    </span>
                <?php endif; ?>
                <i class="fas fa-chevron-<?php echo e(request()->routeIs('admin.paiements.*') ? 'up' : 'down'); ?> small"></i>
            </a>
            <div class="collapse <?php echo e(request()->routeIs('admin.paiements.*') ? 'show' : ''); ?>" id="paiementsMenu">
                <ul class="nav flex-column ms-4 py-2">
                    <li class="nav-item">
                        <a class="nav-link d-flex align-items-center <?php echo e(request()->routeIs('admin.paiements.index') && !request()->has('statut') ? 'active' : ''); ?>" 
                           href="<?php echo e(route('admin.paiements.index')); ?>">
                            <i class="fas fa-list-ul me-2"></i>
                            <span>Tous les versements</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link d-flex align-items-center <?php echo e(request()->get('statut') === 'soumis' ? 'active' : ''); ?>" 
                           href="<?php echo e(route('admin.paiements.index', ['statut' => 'soumis'])); ?>">
                            <i class="fas fa-clock me-2"></i>
                            <span>En attente de validation</span>
                            <?php if(($nbPaiementsPending ?? 0) > 0): ?>
                                <span class="badge bg-warning rounded-pill ms-auto"><?php echo e($nbPaiementsPending); ?></span>
                            <?php endif; ?>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link d-flex align-items-center <?php echo e(request()->get('statut') === 'validé' ? 'active' : ''); ?>" 
                           href="<?php echo e(route('admin.paiements.index', ['statut' => 'validé'])); ?>">
                            <i class="fas fa-check-circle me-2"></i>
...
</style><?php /**PATH C:\Projetsw\sif-project\resources\views/backoffice/layouts/sidebar.blade.php ENDPATH**/ ?>