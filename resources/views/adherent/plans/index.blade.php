    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="text-end">
                    <small class="text-muted">{{ $plans->count() }} plan(s) disponible(s)</small>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <form method="GET" id="filterForm" class="row g-3 align-items-end">
                        <div class="col-md-3">
                            ...
                            <select name="periodicite" id="periodicite" class="form-select form-select-sm" onchange="document.getElementById('filterForm').submit();">
                                ...
                                <option value="semestriel" {{ request('periodicite') == 'semestriel' ? 'selected' : '' }}>Semestriel</option>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="number" name="montant" id="montant" class="form-control form-control-sm" 
                                   value="{{ request('montant') }}" placeholder="Ex: 100000" 
                                   onchange="document.getElementById('filterForm').submit();">
                        </div>
                        <div class="col-md-3">
                            <a href="{{ route('adherent.plans.index') }}" class="btn btn-outline-secondary btn-sm">
            ...
            @foreach($plans as $plan)
                <div class="col-lg-4 col-md-6 mb-4">
                    <div class="card h-100 plan-card" data-plan-id="{{ $plan->id }}">
                        ...
                        <!-- Corps de la carte -->
                        <div class="card-body">
                            <p class="card-text text-muted mb-3">{{ Str::limit($plan->description, 100) }}</p>
                            
                            <!-- Informations financières -->
                            <div class="row text-center mb-3">
                                <div class="col-6">
                                    <div class="border-end">
                                        <div class="h6 text-success mb-0">{{ $plan->taux_interet }}%</div>
                                        <small class="text-muted">Taux d'intérêt</small>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="h6 text-info mb-0">{{ ucfirst($plan->periodicite) }}</div>
                            ...
                            <div class="bg-light rounded p-3 mb-3">
                                <div class="row text-center">
                                    <div class="col-6">
                                        ...
                                        <div class="fw-bold text-dark">{{ number_format($plan->montant_min, 0, ',', ' ') }} FCFA</div>
                                    </div>
                                    <div class="col-6">
                                        <div class="text-muted small">Montant maximum</div>
                                        <div class="fw-bold text-dark">{{ number_format($plan->montant_max, 0, ',', ' ') }} FCFA</div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        @if($plan->frais_adhesion > 0)
                                            <small class="text-muted">
                                                Adhésion: <span class="text-dark">{{ number_format($plan->frais_adhesion) }} FCFA</span>
                                            </small>
                                        @endif
                                        @if($plan->frais_retrait > 0)
                                            <small class="text-muted">
                                                Retrait: <span class="text-dark">{{ number_format($plan->frais_retrait) }} FCFA</span>
                                            </small>
                                        @endif
                                    </div>
                                </div>
                            ...
                            @if($plan->duree_min_jours)
                                <div class="mb-3">
                                    <small class="text-muted">
                                        ...
                                        Engagement minimum : {{ $plan->duree_min_jours }} jours
                                    </small>
                                </div>
                            @endif
                            ...
                            @if($mesAdhesions->count() > 0)
                                <div class="alert alert-info py-2 mb-3">
                                    <small>
                                        ...
                                        Vous avez {{ $mesAdhesions->count() }} adhésion(s) à ce plan
                                        @if($mesAdhesions->where('statut', 'actif')->count() > 0)
                                            ({{ $mesAdhesions->where('statut', 'actif')->count() }} active(s))
                                        @endif
                                    </small>
                                </div>
                            @endif
                        ...
                        <div class="card-footer bg-white border-top">
                            <div class="d-grid gap-2">
                                <a href="{{ route('adherent.plans.show', $plan) }}" class="btn btn-outline-primary btn-sm">
                                    <i class="fas fa-eye me-1"></i>Voir les détails
                                </a>
                                @if($mesAdhesions->where('statut', 'actif')->count() === 0)
                                    <a href="{{ route('adherent.adhesions.create', ['plan' => $plan->id]) }}" class="btn btn-primary btn-sm">
                                        <i class="fas fa-plus-circle me-1"></i>Souscrire à ce plan
                                    </a>
                                @else
                                    <a href="{{ route('adherent.adhesions.index', ['plan' => $plan->id]) }}" class="btn btn-success btn-sm">
            ...
            <div class="col-12">
                <div class="text-center py-5">
                    ...
                    <p class="text-muted mb-4">
                        @if(request()->hasAny(['type', 'periodicite', 'montant']))
