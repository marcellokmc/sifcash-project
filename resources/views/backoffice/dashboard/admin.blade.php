        ...
        <div class="row align-items-center">
            <div class="col-lg-8">
                ...
                <p class="lead mb-0" style="opacity: 0.9;">
                    📊 Gestion centralisée et monitoring en temps réel - {{ now()->format('d/m/Y') }}
                </p>
            </div>
            <div class="col-lg-4 text-end">
                <div class="d-flex align-items-center justify-content-end gap-3">
                    ...
                    <div class="text-center">
                        <div class="h4 fw-bold mb-0">{{ $stats['total_adherents'] ?? 0 }}</div>
    ...
    <div class="col-xl-3 col-md-6">
        <div class="modern-stat-card primary">
            <div class="card-body p-4 text-center">
                ...
                <h2 class="fw-bold mb-1" style="color: #667eea;">{{ $stats['total_users'] }}</h2>
    ...
    <div class="col-xl-3 col-md-6">
        <div class="modern-stat-card success">
            <div class="card-body p-4 text-center">
                ...
                <h2 class="fw-bold mb-1" style="color: #11998e;">{{ $stats['total_adherents'] ?? 0 }}</h2>
    ...
    <div class="col-xl-3 col-md-6">
        <div class="modern-stat-card warning">
            <div class="card-body p-4 text-center">
                ...
                <h2 class="fw-bold mb-1" style="color: #f59e0b;">{{ $stats['documents_en_attente'] ?? 0 }}</h2>
                <h6 class="text-muted mb-2">📄 Documents à Valider</h6>
                @if(isset($stats['documents_en_attente']) && $stats['documents_en_attente'] > 0)
                    <a href="{{ route('admin.validation.documents') }}" class="btn btn-sm rounded-pill px-3" style="background: rgba(245, 158, 11, 0.1); color: #f59e0b; border: 2px solid rgba(245, 158, 11, 0.2); text-decoration: none; font-weight: 600;">
                        <i class="fas fa-eye me-1"></i>👁️ Voir
                    </a>
                @endif
                <div class="progress mt-2" style="height: 6px;">
                    <div class="progress-bar" style="width: {{ min(($stats['documents_en_attente'] ?? 0) * 10, 100) }}%; background: linear-gradient(135deg, #f59e0b 0%, #f97316 100%);"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-xl-3 col-md-6">
        <div class="modern-stat-card info">
            <div class="card-body p-4 text-center">
                ...
                <h2 class="fw-bold mb-1" style="color: #06b6d4;">{{ $stats['ayants_droit_en_attente'] ?? 0 }}</h2>
                <h6 class="text-muted mb-2">👨‍👩‍👧‍👦 Ayants Droit à Valider</h6>
                @if(isset($stats['ayants_droit_en_attente']) && $stats['ayants_droit_en_attente'] > 0)
                    <a href="{{ route('admin.validation.ayants-droit') }}" class="btn btn-sm rounded-pill px-3" style="background: rgba(6, 182, 212, 0.1); color: #06b6d4; border: 2px solid rgba(6, 182, 212, 0.2); text-decoration: none; font-weight: 600;">
                        <i class="fas fa-eye me-1"></i>✓ Valider
                    </a>
                @endif
                <div class="progress mt-2" style="height: 6px;">
                    <div class="progress-bar" style="width: {{ min(($stats['ayants_droit_en_attente'] ?? 0) * 15, 100) }}%; background: linear-gradient(135deg, #06b6d4 0%, #3b82f6 100%);"></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="modern-action-card">
            <div class="card-body p-4 position-relative">
                ...
                <div class="d-grid gap-3">
                    <a href="{{ route('admin.adherents.index') }}" class="action-btn">
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            @if(isset($stats['adherents_en_attente']) && $stats['adherents_en_attente'] > 0)
                                <span class="badge" style="background: rgba(245, 158, 11, 0.8); color: white; font-weight: 600;">{{ $stats['adherents_en_attente'] }}</span>
                            @endif
                        </div>
                    </a>
                    
                    <a href="{{ route('admin.validation.documents') }}" class="action-btn">
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                            ...
                            @if(isset($stats['documents_en_attente']) && $stats['documents_en_attente'] > 0)
                                <span class="badge" style="background: rgba(245, 158, 11, 0.8); color: white; font-weight: 600;">{{ $stats['documents_en_attente'] }}</span>
                            @endif
                        </div>
                    </a>
                    
                    <a href="{{ route('admin.validation.ayants-droit') }}" class="action-btn">
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                            ...
                            @if(isset($stats['ayants_droit_en_attente']) && $stats['ayants_droit_en_attente'] > 0)
                                <span class="badge" style="background: rgba(6, 182, 212, 0.8); color: white; font-weight: 600;">{{ $stats['ayants_droit_en_attente'] }}</span>
                            @endif
                        </div>
                    </a>
                    
                    <a href="{{ route('admin.types-documents.index') }}" class="action-btn">
        ...
        <div class="activity-card">
            ...
            <div class="card-body p-4">
                                ...
                                <td style="border: none; padding: 1rem 0.75rem;">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            <div class="fw-bold" style="color: #2d3748;">{{ $log->user->name }}</div>
                                            <small style="color: #718096;">{{ $log->user->email }}</small>
                                        </div>
                                    </div>
                                </td>
                                <td style="border: none; padding: 1rem 0.75rem;">
                                    <span class="badge rounded-pill px-3 py-2" style="background: linear-gradient(135deg, 
                                        {{ $log->action === 'login' ? '#11998e, #38ef7d' : ($log->action === 'logout' ? '#f59e0b, #f97316' : '#06b6d4, #3b82f6') }}
                                        ); color: white; font-weight: 600;">
                                        <i class="fas fa-{{ $log->action === 'login' ? 'sign-in-alt' : ($log->action === 'logout' ? 'sign-out-alt' : 'info-circle') }} me-1"></i>
                                        {{ ucfirst($log->action) }}
                                    </span>
                                </td>
                                <td style="border: none; padding: 1rem 0.75rem;">
                                    <div style="color: #4a5568;">
                                        <div><i class="fas fa-globe me-2" style="color: #667eea;"></i>{{ $log->ip_address }}</div>
                                        @if($log->user_agent)
                                            <small style="color: #718096;"><i class="fas fa-desktop me-1"></i>{{ Str::limit($log->user_agent, 25) }}</small>
                                        @endif
                                    </div>
                                </td>
                                <td style="border: none; padding: 1rem 0.75rem;">
                                    <div style="color: #4a5568;">
                                        ...
                                        <small style="color: #718096;">{{ $log->created_at->format('H:i:s') }}</small>
                                    </div>
                                </td>
                            </tr>
                            @empty
                ...
                <div class="mt-4 text-center">
                    <a href="{{ route('admin.logs.connexions') }}" class="btn btn-lg rounded-pill px-4" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; font-weight: 600;">
...
@if(isset($stats['documents_en_attente']) && $stats['documents_en_attente'] > 0 || isset($stats['ayants_droit_en_attente']) && $stats['ayants_droit_en_attente'] > 0)
<div class="row g-4 mt-4">
    <div class="col-12">
                        ...
                        <div class="p-3 rounded-3" style="background: rgba(255,255,255,0.1); backdrop-filter: blur(10px);">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <div class="h5 fw-bold mb-1">{{ $stats['documents_en_attente'] }}</div>
                                        <small style="opacity: 0.8;">📄 Document(s) à valider</small>
                                    </div>
                                </div>
                                <a href="{{ route('admin.validation.documents') }}" class="btn btn-light btn-lg rounded-pill px-4">
                        ...
                        <div class="p-3 rounded-3" style="background: rgba(255,255,255,0.1); backdrop-filter: blur(10px);">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <div class="h5 fw-bold mb-1">{{ $stats['ayants_droit_en_attente'] }}</div>
                                        <small style="opacity: 0.8;">👨‍👩‍👧‍👦 Ayant(s) droit à valider</small>
                                    </div>
                                </div>
                                <a href="{{ route('admin.validation.ayants-droit') }}" class="btn btn-light btn-lg rounded-pill px-4">
...
<script>
    ...
    @if(isset($stats['adherents_actifs']) && isset($stats['adherents_inactifs']) && isset($stats['adherents_en_attente']))
    ...
    new Chart(adherentCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: [
                    {{ $stats['adherents_actifs'] ?? 0 }},
                    {{ $stats['adherents_en_attente'] ?? 0 }},
                    {{ $stats['adherents_inactifs'] ?? 0 }}
                ],
                backgroundColor: [
