<div class="position-sticky pt-3">
    ...
    <ul class="nav flex-column">
        ...
        <!-- Section Adhérents -->
        <li class="nav-item">
            <a class="nav-link d-flex align-items-center {{ request()->routeIs('admin.adherents.*') ? 'active' : '' }}" 
               href="{{ route('admin.adherents.index') }}">
                <i class="fas fa-users me-3"></i>
                <span>Adhérents</span>
                @php /* compteur inline pour éviter erreurs de parsing */ @endphp
                <span class="badge bg-warning rounded-pill ms-auto {{ (\App\Models\Adherent::where('statut_compte','en_attente_de_verification')->count() > 0) ? '' : 'd-none' }}">
                    {{ \App\Models\Adherent::where('statut_compte','en_attente_de_verification')->count() }}
                </span>
            </a>
        </li>
        ...
        <li class="nav-item">
            @php($nbAdhesionsEnAttenteSidebar = \App\Models\Adhesion::where('statut', 'en_attente_activation')->count())
                    ...
                    <li class="nav-item mt-2 pt-2 border-top">
                        <a class="nav-link d-flex align-items-center {{ request()->routeIs('admin.adhesions.*') ? 'active' : '' }}" 
                           href="{{ route('admin.adhesions.index') }}">
                            ...
                            @php($nbAdhesionsActives = \App\Models\Adhesion::where('statut', 'actif')->count())
                    ...
                    <li class="nav-item">
                        <a class="nav-link d-flex align-items-center" 
                           href="{{ route('admin.adhesions.index', ['statut' => 'en_attente_activation']) }}">
                            <i class="fas fa-clock me-2"></i>
                            <span>En attente</span>
                            @php($nbAdhesionsEnAttente = \App\Models\Adhesion::where('statut', 'en_attente_activation')->count())
                    ...
                    <li class="nav-item">
                        <a class="nav-link d-flex align-items-center" 
                           href="{{ route('admin.adhesions.index', ['statut' => 'suspendue']) }}">
                            ...
                            @php($nbAdhesionsSuspendues = \App\Models\Adhesion::where('statut', 'suspendue')->count())
                    ...
                    <li class="nav-item">
                        <a class="nav-link d-flex align-items-center {{ request()->routeIs('admin.retraits.index') ? 'active' : '' }}" 
                           href="{{ route('admin.retraits.index') }}">
                            ...
                            @php($nbRetraitsEnAttente = \App\Models\DemandeRetrait::where('statut', 'en_attente')->count())
        ...
        <li class="nav-item">
            <a class="nav-link d-flex align-items-center {{ request()->routeIs('admin.notifications.*') ? 'active' : '' }}" 
               href="{{ route('admin.notifications.index') }}">
                ...
                <span class="ms-auto {{ (auth()->check() && method_exists(auth()->user(), 'unreadNotifications') && auth()->user()->unreadNotifications->count() > 0) ? '' : 'd-none' }}" data-notification-badge>
                    {{ auth()->check() && method_exists(auth()->user(), 'unreadNotifications') ? (auth()->user()->unreadNotifications->count() > 99 ? '99+' : auth()->user()->unreadNotifications->count()) : '' }}
                </span>
            </a>
        </li>
