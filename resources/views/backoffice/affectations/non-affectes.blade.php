...
@section('title', 'Adhérents non affectés')
@section('page-title', 'Adhérents Non Affectés')
            ...
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $adherents->total() }}</div>
            ...
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        ...
                        <div class="h5 mb-0 font-weight-bold text-gray-800">{{ $agences->count() }}</div>
...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <!-- Barre de recherche -->
        <form method="GET" action="{{ route('admin.affectations.non-affectes') }}" class="mb-4">
            <div class="row g-3">
                <div class="col-md-9">
                ...
                <div class="col-md-3">
                    <div class="btn-group w-100" role="group">
                        ...
                        <a href="{{ route('admin.affectations.non-affectes') }}" class="btn btn-outline-secondary">
        ...
        <div class="border-top pt-4">
            <form method="POST" action="{{ route('admin.affectations.affecter-masse') }}" id="formAffectationRapide">
                @csrf
                    ...
                    <div class="col-md-5">
                        ...
                        <select name="agence_id" id="agence_rapide" class="form-select form-select-lg">
                            ...
                            @foreach($agences as $agence)
                                <option value="{{ $agence->id }}" data-ville="{{ $agence->ville ?? '' }}">
                                    {{ $agence->nom }}
                                    @if($agence->ville) - {{ $agence->ville }}@endif
                                </option>
                            @endforeach
                        </select>
                                ...
                                @foreach($agents as $agent)
                                    <div class="form-check agent-item-rapide p-2 rounded hover-bg-white" 
                                        ...
                                        <input class="form-check-input agent-checkbox-rapide" 
                                               ...
                                               name="agent_ids[]" 
                                               value="{{ $agent->id }}" 
                                               id="agent_rapide_{{ $agent->id }}">
                                        <label class="form-check-label w-100" for="agent_rapide_{{ $agent->id }}" style="cursor: pointer;">
    ...
    <div class="card-header py-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap">
            <h6 class="m-0 font-weight-bold text-danger">
                ...
                <span class="badge bg-danger">{{ $adherents->total() }}</span>
            </h6>
                    ...
                    <tr class="align-middle">
                        <td class="text-center">
                            <input type="checkbox" class="form-check-input adherent-check" 
                                   value="{{ $adherent->id }}" style="width: 20px; height: 20px;">
                        </td>
                        <td>
                            <strong class="text-primary">{{ $adherent->membre_id }}</strong>
                        </td>
                        <td>
                            <a href="{{ route('admin.adherents.show', $adherent) }}" 
                               class="text-decoration-none fw-bold text-dark">
                                <i class="fas fa-user me-1"></i>{{ $adherent->nom_complet }}
                            </a>
                        </td>
                        <td>
                            <div>
                                <i class="fas fa-phone text-muted me-1"></i>
                                <small>{{ $adherent->telephone ?? 'N/A' }}</small>
                            </div>
                            @if($adherent->email)
                            <div>
                                <i class="fas fa-envelope text-muted me-1"></i>
                                <small class="text-muted">{{ Str::limit($adherent->email, 20) }}</small>
                            </div>
                            @endif
                        </td>
                        <td>
                            @if($adherent->agence)
                                <span class="badge bg-info text-white">
                                    <i class="fas fa-building me-1"></i>{{ $adherent->agence->nom }}
                                </span>
                            @else
                        ...
                        <td>
                            @if($adherent->agentGestionnaire)
                                <span class="badge bg-secondary">
                                    <i class="fas fa-user-tie me-1"></i>{{ $adherent->agentGestionnaire->name }}
                                </span>
                            @else
                                <span class="badge bg-danger">
            ...
            <div class="p-3 border-top bg-light">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="text-muted">
                        Affichage de {{ $adherents->firstItem() }} à {{ $adherents->lastItem() }} sur {{ $adherents->total() }} résultats
                    </div>
                    <div>
                        {{ $adherents->links() }}
                    </div>
